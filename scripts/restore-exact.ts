...
// Top up to line 226 (inclusive, 0-indexed: 226)
const head = lines.slice(0, 226).join('\n')

// Middle: clean refineTranscriptsAndSpeakers closure + clean analyzeCall prompt
const middle = `        .map((s) => \`\${s.speaker === 'MANAGER' ? 'Менежер' : 'Мижоз'}: \${s.text}\`)
        .join('\\n')
...
// Tail from line 359 (`let attempts = 0`) to end
const tail = lines.slice(358).join('\n')

fs.writeFileSync('src/lib/ai/openai.provider.ts', head + '\n' + middle + '\n' + tail, 'utf-8')
console.log('Successfully restored and upgraded openai.provider.ts!')
